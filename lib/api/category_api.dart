...
class CategoryApi {
  ...
  Future<Category> getCategoryById(int categoryId) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/api/categories/$categoryId'), // CORREGIDO
        headers: _headers,
      );
  ...
  Future<Category> createCategory(
    ...
    try {
      final response = await http.post(
        ...
        ), // CORREGIDO: agregado /api/categories
  ...
  Future<bool> deleteCategory(int categoryId, String token) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        throw Exception(
          errorData['message'] ?? 'No se puede eliminar la categoría',
        );
      } else {
  ...
  Future<Category> deactivateCategory(int categoryId, String token) async {
    try {
      final response = await http.put(
        Uri.parse(
          '$baseUrl/api/categories/$categoryId/deactivate',
        ), // CORREGIDO
        headers: _authHeaders(token),
      );
  ...
  Future<Category> activateCategory(int categoryId, String token) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/api/categories/$categoryId/activate'), // CORREGIDO
        headers: _authHeaders(token),
      );
  ...
  Future<List<Product>> getCategoryProducts(int categoryId) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/api/categories/$categoryId/products'), // CORREGIDO
        headers: _headers,
      );
  ...
  Future<CategoryStatistics> getCategoryStatistics(int categoryId) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/api/categories/$categoryId/stats'), // CORREGIDO
        headers: _headers,
      );
  ...
  Future<List<Category>> searchCategories(String searchTerm) async {
    try {
      final response = await http.get(
        Uri.parse(
          '$baseUrl/api/categories/search?q=${Uri.encodeComponent(searchTerm)}',
        ), // CORREGIDO
        headers: _headers,
      );
  ...
  Future<List<Category>> getCategoriesWithProducts() async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/api/categories/with-products'), // CORREGIDO
        headers: _headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Error al obtener categorías con productos: ${response.statusCode}',
        );
      }
    } catch (e) {
      throw Exception('Error de conexión: $e');
