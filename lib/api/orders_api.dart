...
class OrdersApi {
  ...
  Future<List<Order>> getAllOrders({
    ...
    try {
      ...
      if (fechaInicio != null) {
        queryParams.add('fecha_inicio=${fechaInicio.toIso8601String().split('T')[0]}');
      }
      if (fechaFin != null) {
        queryParams.add('fecha_fin=${fechaFin.toIso8601String().split('T')[0]}');
      }
  ...
  Future<Order> cancelOrder(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = json.decode(response.body);
        throw Exception(errorData['message'] ?? 'No se puede cancelar este pedido');
      } else {
  ...
  Future<List<OrderHistory>> getOrderHistory(String orderId, String token) async {
    try {
      final response = await http.get(
